...
try {
    ...
    if(isset($_SERVER['argv']) && !empty($_SERVER['argv'])){
        foreach ($_SERVER['argv'] as $arg) {
            if (preg_match('/^--(.*)$/', $arg, $match) && $match[1] == 'client-code') {
                $client_code = true;
            } else {
                if ($client_code ===  true) {
                    $client_code = $arg;
    ...
    if(!$client_code){
        echo 'MISSING client-code param'."\n";
        exit(1);
    }

    // Set client code
    $_SERVER['CLIENT_CODE'] = $client_code;
    $app = Mage::app('default','store',['config_model'=>'HMP_MageMulti_Model_Mage_Core_Config']);
